...
class DataframeSerializer:
    ...
    def serialize(cls, df: "DataFrame", dialect: str = "postgres") -> str:
        ...
        # Add description attribute if available (check if schema exists for backward compatibility)
        if hasattr(df, 'schema') and hasattr(df.schema, 'description') and df.schema.description is not None:
            dataframe_info += f' description="{df.schema.description}"'

        if hasattr(df, 'schema') and hasattr(df.schema, 'columns') and df.schema.columns:
            columns = [column.model_dump() for column in df.schema.columns]
            dataframe_info += f' columns="{json.dumps(columns, ensure_ascii=False)}"'

        dataframe_info += f' dimensions="{df.shape[0]}x{df.shape[1]}">'
