...
from dataai.query_builders.sql_parser import SQLParser
...
class CodeCleaner:
    ...
    def _clean_sql_query(self, sql_query: str) -> str:
        ...
        # Build allowed table names mapping
        allowed_table_names = {}
        for df in self.context.dfs:
            table_name = df._table_name if hasattr(df, '_table_name') and df._table_name else 'df'
            allowed_table_names[table_name] = table_name
            allowed_table_names[f'"{table_name}"'] = table_name
